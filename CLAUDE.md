...
| `disparity_reversion` | 단기 과매도 평균회귀 — 이격도 < 96% 반등 | mean_reversion | sideways |
| ~~`institutional_flow`~~ | ~~수급 기반~~ → 스크리닝 필터로 흡수 | - | - |
...
adaptive 모드 (config.yaml strategy.type: adaptive)
  trending:  momentum_pullback (수급 가산점은 스크리닝에서 적용)
  sideways:  disparity_reversion
  bearish:   매수 차단

포지션 스케일: trending 100% / sideways 50% / bearish 0%
기존 전략 + institutional_flow는 src/strategy/에 파일 보존, __init__.py에서 비활성
```
