...
수술적 정밀 수정(surgical fix) Phase 1~2 완료.
...
| ID | 상태 | 내용 |
|----|------|------|
| FIX-1 | 완료 | 거래세 0.2% → 0.15% 전체 일괄 변경 (7곳) |
| FIX-2 | 완료 | 백테스트 비용 모델을 config.yaml에서 읽도록 변경 |
| FIX-3 | 완료 | 슬리피지를 체결가 조정 방식으로 분리 |
| FIX-4 | 완료 | Walk-Forward --codes 인자 추가 |
| STRAT-1 | 완료 | 파라미터 기본값 4곳 통일 + max_hold_days 경로 버그 |
| FIX-6 | 완료 | 포지션 사이징 하드코딩 → config 연동 |
| FIX-7 | 완료 | 실험 스크립트 archived/ 정리 |
| STRAT-2 | 완료 | 백테스트 ↔ 실전 청산 로직 일치 |
| STRAT-3 | 완료 | Adaptive 진입 전략 추적 — entry_strategy 필드 |
| INFRA-1 | 완료 | StopManager fallback 기본값 config.yaml과 통일 |
| INFRA-2 | 완료 | _evaluate_exit → _check_strategy_exit 분리 |
| INFRA-3 | 완료 | Paper 모드 OrderManager 이중 안전장치 |
| INFRA-5 | 완료 | DataStore.get_trade_statistics() 구현 |

### Phase 4 전략 재설계 (Phase A 완료 후 실행)
