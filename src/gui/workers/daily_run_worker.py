...
단계:
  1/4 신규 상장 감지       detect_new_listings.main()
  2/4 일봉 증분           collect_daily_candles.main(incremental=True)
  3/4 시총 증분           collect_market_cap.main()
  4/4 지수 갱신           collect_index_daily.main() (sys.argv 임시 치환)

시그널 생성은 별도 단계 없음 — TradingEngine이 장전 09:00 스크리닝에서
v2.4 종목 선별을 담당. (이전 5/5 단계 Orchestrator는 archive로 이동됨)
...
class DailyRunWorker(QThread):
    ...
    def _steps(self) -> list[tuple[str, Callable[[], None], bool]]:
        ...
        """
        return [
            ("1/4 신규 상장 감지", self._step_detect_new_listings, False),
            ("2/4 일봉 증분",     self._step_daily_candles,        True),
            ("3/4 시총 증분",     self._step_market_cap,           True),
            ("4/4 지수 갱신",     self._step_index_daily,          False),
        ]
